...
if platform.system() == 'Windows':
    handle = ctypes.CDLL("nvinfer_plugin_10.dll", mode=ctypes.RTLD_GLOBAL)
else:
...
if platform.system() == 'Windows':
    handle = ctypes.CDLL(".\LayerNormPluginBasic\LayerNorm.dll", mode=ctypes.RTLD_GLOBAL)
else:
...
def emb_layernorm(network_helper, config, weights_dict, builder_config, sequence_lengths, batch_sizes):
    ...
    input_ids = network_helper.addInput(name="input_ids", dtype=trt.int64, shape=(1, -1))
    token_type_ids = network_helper.addInput(name="token_type_ids", dtype=trt.int64, shape=(1, -1))
    position_ids = network_helper.addInput(name="position_ids", dtype=trt.int64, shape=(1, -1))
...
def test_case_data(infer_helper, case_data_path):
    ...
    input_ids = case_data['input_ids'].astype(np.int64)
    token_type_ids = case_data['token_type_ids'].astype(np.int64)
    position_ids = case_data['position_ids'].astype(np.int64)
    print(input_ids)
