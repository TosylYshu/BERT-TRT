...
class TrtNetworkHelper():
    ...
    def addLayerNorm(self, x, gamma, beta, use_fp16, layer_name=None, precision=None):
        ...
        if not plugin:
            return RuntimeError("Could not create_plugin LayerNormPluginDynamic")
    ...
    def addLinear(self, x, weight, bias, layer_name=None, precision=None):
        ...
        filter_const = self.network.add_constant(weight.shape, trt.Weights(weight))
        mm = self.network.add_matrix_multiply(
            x,
        ...
        bias = np.array([[bias]])
        bias_const = self.network.add_constant(bias.shape, trt.Weights(bias))
        trt_layer = self.network.add_elementwise(mm.get_output(0), bias_const.get_output(0), trt.ElementWiseOperation.SUM)
        self.layer_post_process(trt_layer, layer_name, precision)
...
class InferHelper():
    ...
    def infer(self, inputs: list):
        ...
        for i in range(nInput):
            ...
            self.context.set_input_shape(self.engine.get_tensor_name(i), tuple(inputs[i].shape))
            # print(inputs[i].nbytes)

        # for i in range(0, self.engine.num_io_tensors):
            # tensor_name = self.engine.get_tensor_name(i)
            # print("get_tensor_shape:" + str(self.context.get_tensor_shape(tensor_name)))

        outputs = []
        for i in range(len(inputs), self.engine.num_io_tensors):
            tensor_name = self.engine.get_tensor_name(i)
            outputs.append(np.zeros(self.context.get_tensor_shape(tensor_name)).astype(np.float32))
        ...
        for i in range(len(inputs), self.engine.num_io_tensors):
            tensor_name = self.engine.get_tensor_name(i)
            trt_output_shape = self.context.get_tensor_shape(tensor_name)
        ...
        # warm up
        noerror = self.context.execute_v2(bufferD)
        if not noerror:
